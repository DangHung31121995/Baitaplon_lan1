<?php
  ...
  class HISTORYBOOKING_CONTROLLER
  {
  	    private $model;
		public function __construct() {
			require ('Model/HistoryBooking/HistoryBookingViewModel.php');	
				
			$this->model = new HISTORYBOOKING_MODEL();
			...
			switch($action) 
			{
             ...
             case 'update':
              ...
              break;
                ...
                if($action_id !=null)
                {
                    $result=$this->model->delete_history($action_id);
                    if ($result)
                  {					
                    header("Location: index.php?controller=HISTORYBOOKING&action=listhistory");
                  }
              ...
              break;
				...
				// 
				 ...
				 break;
				case 'listhistory':
				 $result=$this->model->GETALLHISTORY();
				 $sumpage= count($result);
				 $numbercurrentpage=2;
				 $numberpage=ceil($sumpage/$numbercurrentpage);
				 $trang = isset($_GET['trang'])?$_GET['trang']:'';
				 if(empty($trang ))
				 {
                    ...
                    $tong=1;
					$tong=($trang-1)* $numbercurrentpage;
					$result1=$this->model->PagingHistory($tong,$numbercurrentpage);
				 
			
					
				 $check='no';
				 require("View/HistoryBooking/Index.php");
				 //$trang=$_GET['trang'];
