	...
	public function run(){
		...
		switch ($action) {
			...
			case 'tttk':
		        ...
		        header('location:index.php?controller=viewaccount&action=tttk');
			...
			case 'changepass':
		        ...
		        $pass_2 = $_POST['password_new_2'];
		        if(empty($pass)){
		        	echo "<script>alert('Không được để trống'); history.back(-1);</script>";
		        }
		        elseif(strcmp($pass,$pass_2)!=0  ){
		        	echo "<script>alert('Mật Khẩu Không trùng khớp'); history.back(-1);</script>";
		        	break;
		        }else{
			        $check =  $this->model->updatePass($username,$pass);
			        // print(gettype($check));
			...
			case 'history':
				...
				if(!empty($idHisotry)){
					...
					$idUser = $this->model->getUser($username)->id;

				
					$historybooking = $history->getWithUser($idUser); // $idUser =12;
					if(empty($historybooking)){
						
						require_once("View/user/history_empty.php");
