...
class TYPEROOM_MODEL{
	...
	{
		$mySQL  = "UPDATE roomtype SET typeName='{$loaiphong->typeName}',pricePerDay={$loaiphong->pricePerDay},typeDescription='{$loaiphong->typeDescription}',totalPeople={$loaiphong->totalPeople},image='{$loaiphong->image}' where id={$loaiphong->id}";
		//die ($mySQL);
	...
	public function delete_typeroom($id)
	{
	 ...
	 public function insert(data_entity $loaiphong)
	 {
		...
		// $new_typerom->image=$destination;
		  ...
		  $mySQL = "INSERT INTO roomtype (id,typeName,pricePerDay,typeDescription,totalPeople,image) VALUE (NULL,'{$loaiphong->typeName}',{$loaiphong->pricePerDay},'{$loaiphong->typeDescription}',{$loaiphong->totalPeople},'{$loaiphong->image}')";
